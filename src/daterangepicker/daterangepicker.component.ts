...
export class DaterangepickerComponent implements OnInit {
  ...
  showQuarterOnYear: Boolean = false;
  ...
  generateYearly() {
    ...
    this.showYears = true;
    let count = 0, temp = [];
    for (let year = this.yearStartValue; year <= this.yearStartValue+25; year += 1) {
      if (count < 5) {
        temp.push(year);
        count += 1;
      }
      if (count === 5) {
        this.yearsArr.push(temp);
        temp = [];
        count = 0;
      }
    }
  ...
  generatePreviousYears() {
    this.yearStartValue = this.yearStartValue - 25;
    this.generateYearly();
  }
  generateNextYears() {
    this.yearStartValue = this.yearStartValue + 25;
  ...
  selectYear(year){
    this.showQuarterOnYear = true;
    this.currentYear = year;
    this.showYears = false;
  }

  selectQuarter(start, end) {

    const DATE = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
    const startDate = `${this.currentYear}-${start}-1`;
    const endDate = `${this.currentYear}-${end}-${DATE[end-1]}`;
    console.log('====================================');
    console.log({ chosenLabel: '', startDate: new Date(startDate), endDate: new Date(endDate) });
    console.log('====================================');
    this.choosedDate.emit({ chosenLabel: '', startDate: new Date(startDate), endDate: new Date(endDate) });
  }
}
